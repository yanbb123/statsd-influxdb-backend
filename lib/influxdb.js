...
function InfluxdbBackend (startupTime, config, events) {
  ...
  } else if (self.version >= 0.9 || self.version < 0.11) {
...
function influxKey (key, type) {
  ...
  return name + '_' + type + (tags.length > 0 ? ',' + tags.join(',') : '')
}
  ...
  for (key in timerData) {
    ...
    if (timerMetrics.histogram) {
      ...
      for (histoKey in histoMetrics) {
        let value = histoMetrics[histoKey]
        let k = influxKey(key, 'timer_histogram_' + histoKey)
    ...
    for (timerKey in timerMetrics) {
      let value = timerMetrics[timerKey]
      let k = influxKey(key, 'timer' + '_' + timerKey)
...
InfluxdbBackend.prototype.httpPOST_v09 = function (points) {
  ...
  var self = this
  var query = { u: self.username, p: self.password, db: self.database }
  var protocolName = self.protocol === http ? 'HTTP' : 'HTTPS'
  var startTime
